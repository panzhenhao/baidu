...
class rsSpider(scrapy.spiders.Spider):  # 该类继承自 scrapy 中的 spider
    ...
    def parse(self, response):
        ...
        pageUrl = "http://baike.baidu.com/view/"+''.join(response.xpath("//div[@class='anchor-list'][1]/a[2]").re('name="(.*?)"')).split("_")[0].replace("sub","")+".htm"   #.re('href="(.*?)"')[0]  # 解析爬取网页的 url，并不是直接使用函数获取，那样会夹杂乱码
